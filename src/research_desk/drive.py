...
def _google_services():
    ...
    return build("drive", "v3", credentials=credentials), MediaFileUpload, credentials.service_account_email
...
def _find_or_create_folder(service, name: str, parent_id: str) -> str:
    ...
    results = service.files().list(
        q=query, spaces="drive", fields="files(id,name)", pageSize=1, supportsAllDrives=True, includeItemsFromAllDrives=True
    ).execute()
    if results["files"]:
        return results["files"][0]["id"]
    return (
        service.files()
        .create(body={"name": name, "mimeType": "application/vnd.google-apps.folder", "parents": [parent_id]}, fields="id", supportsAllDrives=True)
        .execute()["id"]
    )
...
def _upsert_file(service, media_type, file_path: Path, parent_id: str) -> str:
    ...
    results = service.files().list(
        q=query, spaces="drive", fields="files(id)", pageSize=1, supportsAllDrives=True, includeItemsFromAllDrives=True
    ).execute()
    ...
    return (
        service.files()
        .create(body={"name": file_path.name, "parents": [parent_id]}, media_body=media, fields="id", supportsAllDrives=True)
        .execute()["id"]
    )
...
def publish_directory(source_dir: Path, root_folder_id: str, theme: str, now: datetime | None = None) -> dict[str, str | int]:
    ...
    try:
        ...
        if error.resp.status == 404:
            raise RuntimeError(
                f"Google Drive folder '{root_folder_id}' was not found or is not accessible to the "
                f"service account '{service_account_email}'. Check that the ID is correct and that the "
                "folder (or its Shared Drive) is shared with that service account with at least Content "
                "Manager / Editor access."
            ) from error
        raise
